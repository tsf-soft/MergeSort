...
Комплект состоит из двух `CLI` утилит: `TsfDataPrepare` и `TsfMergeSort`. Это взаимодополняющие утилиты используемые последовательно в случае, когда в качестве входных данных заданы текстовые данные произвольного формата, такие например, как на показанные на <a href="#fig2_1">рис. 2.1</a>, либо  <a href="#fig2_2">рис. 2.2</a>. Утилита `TsfDataPrepare` — приводит такие данные к формату, в котором одно символьное слово занимает одну строку текстового файла, любые пробелы в строках текста отсутствуют. Выходные данные в файлах результата работы этой утилиты — не упорядочены.
...
> [!NOTE]
> _**Примечание:**
> Терминалы интерпретаторов командной строки используемых в Windows, ANSI-решим не поддерживают, поэтому весь вывод утилит в эти терминалы будет монохромным._
...
В разделе <a href="#cli_args">Аргументы командной строки утилит</a> перечислены три категории опций, указываемых в командной строке. Далее, применяемые опции будут описаны подробно.
