...
Как было сказано в разделе 1, утилиты обрабатывают входные файлы полследовательно.
...
```
Эта команда означает, что утилите `TsfDataPrepare` в её исполняемом варианте предлагается обработать все файлы с расшинением `.txt` находящиеся в каталоге `data/raw_data/` относительно каталога местонахождения утилиты, при этом нужно считать содержимое всех файлов символьным (опция `-s`), выполнять обработку пулом из 16 потоков и поместить результаты своей работы ("нормализованные" файлы) в каталог `data/prep_data/` относительно местоположения утилиты. Из перечисленных опций, для этой утилиты "не обязательной" являеся обпия размера пула потоков, поскольку эта утилита по умолчанию пытается задействовать максимально возможное из числа необходимых потоков исполнения и в максимальном своём значении это значение может равняться количеству входных файлов переданных утилите на обработу, т.е. теоретически количество параллельных потоков задействованных этой утилитой может быть более 32.
