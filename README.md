...
В качестве второй и третьей группы параметров, - как указано в разделе 4.2 - используются имена файлов файловой системы Windows. К именам этих аргументов применимы стандартные треования Windows, а именно имена могут задаваться:
...
- простым имем файла (без путей) `MyFile.txt`;
- групповой операций (с использованием мета-символов), например `data\temp_data\*`, при этом будут найдены все файлы с расшинением `.txt` (по умолчанию);
- с неопределённым символом а имени: `temp_data\My??le.txt`;
- иные форматы, допустимые синтаксисом командной строки.
...
При использовании "групповых" операций (метасимволов), каталоги поиска могут содержать разнообразные файлы и вложенные в них каталоги. Утилиты "возьмут" только те, которые удовлетворяют их фильтрам поиска.
...
- путь хранения "сырых" входных файлов, где храняться исходные данные произвольного формата; этот путь используется утилитой `TsfDataPrepare`, она по умолчанию извлекает входные данные для своей работы из этого каталога;
- путь хранения подготовленных данных, где по умолчанию храняться выходные данные результа работы утилиты `TsfDataPrepare`;
...
- путь хранения выходных (отсортированных данных (файлов); сюда (по умолчанию) утилиты `TsfMergeSort` помещает свои выходные файлы с отсортированными файлами.

Для перечисленных категорий файлов, в конфигурации утилит по умолчанию установлены следующие значения:
- `data/raw_data/` - относителный путь хранения "сырых" даннфх (файлов);
