...
Для перечисленных категорий файлов, в конфигурации утилит по умолчанию установлены следующие значения:
...
На рис. 2.3. приведен скриншот конфигурационного файла по умоляанию, используемого утилитами для работы. Продублирую здесь содержимое конфигурационного файла:
...
По сути, это JSON-файл хранящий данные по схеме "Ключ": "Значение" и имеющий расширение `.cfg`. Рассмотрим параметры конфигурации подробно.

- `"dataType"` параметр, определяет тип содержимого файлов, над которыми будут проводится операции обработки; для этого параметра:
  - значение `false` указывает, что входные файлы рассматриваются как содержащие данные в виде целых чисел (это значение по умолчанию);
  - значение `true` указывает, что входные файлы рассматриваются как содержащие символьные данные;
- `"sortOrder"` параметр, определяет направление сортировки к которому должен быть приведён выходной результат; для этого параметра:
  - значение `false` указывает, что файл должен быть отсортирован по возрастанию (это значение по умолчанию);
  - значение `true` указывает, что файл должен быть отсортирован по убыванию;
- `"threadPoolCnt"` параметр, определяет на какое количество фрагментов файл будет разделён с использованием параллельных потоков выполнения; значение по умолчанию для параметра = 4; допустимые значения описаны в разделе ХХ; можно указывать любое значение, однако в конфигурации оно будет приведено к значению числа 2 в степени Х: максимальное значение = 32;
- `"tmpFilePref"` параметр, определяет начальный префикс файла нижнего уровня разделения, т.е. с использованием английского алфавита [A-Z] - можно задействовать слияние сложностью в 32 уровня (2 в 32-ой степени) попарного слияния файлов-фрагментов;
...
- `"minShardFactor"` параметр, определяет минимальное количество строк на один фрагмент, менее которого разделение файла на фрагменты производится не будет; для примера, если при 4-х потоках разделения и 20 строках имнимального размера текстового файла - фактически во входном файле будет мерее 80 строк (20 х 4), то разделение на фрагменты для такого маленького файла производится не будет; значение по умолчанию равно 20-ти строкам;
