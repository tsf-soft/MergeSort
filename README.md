...
Для случая, когда в аргументах командной строки будут содержаться несколько взаимоисключающих опций одной категории, например `-p4 -i -p16`, то к фактическому исполнению будет принята последняя из опций указанной категории, т.е. в нашем случае для категории `Степень распараллеливания` будет установлено значение 16 параллельных потоков исполнения.
