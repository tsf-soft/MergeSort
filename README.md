...
- `"dataType"` параметр, определяет тип содержимого файлов, над которыми будут проводится операции обработки; для этого параметра возможны такие значения:
...
- `"minShardFactor"` параметр, определяет минимальное количество строк на один фрагмент разделения, менее которого разделение файла на фрагменты производится не будет; для примера, если при 4-х потоках разделения и 20 строках имнимального размера текстового файла фактически во входном файле будет мерее 80 строк (20 х 4), то разделение на фрагменты для такого маленького файла производится не будет; значение по умолчанию равно 20-ти строкам;
...
Оставшиеся параметры конфигурационного файла, касающиеся структуры рабочих каталого, а также значения по умолчанию для этих параметров, определены в разделе 5.1.
К таким параметрам относятся:
- `relRawDataPath` - путь к каталогу "сырых" входных данных;
- `relPrepDataPath` - путь к каталогу подготовленных ("нормальизованных") данных;
...
## **6. Работа утилит и комплект поставки**
...
```
Эта команда означает, что утилите `TsfDataPrepare` в её исполняемом варианте (`.exe`) предлагается обработать все файлы с расшинением `.txt` находящиеся в каталоге `data/raw_data/` относительно каталога местонахождения утилиты, при этом ей нужно рассматривать содержимое всех файлов символьным (опция `-s`), выполнять обработку пулом из 16 потоков (опция `-p16`) и поместить результаты своей работы ("нормализованные" файлы) в каталог `data/prep_data/` (значение по умолчанию из конфигурационного файла) относительно местоположения утилиты. Из перечисленных опций, для этой утилиты "не обязательной" являеся опция размера пула потоков, поскольку эта утилита по умолчанию пытается задействовать максимально возможное число из числа необходимых потоков исполнения и в максимальном своём значении это значение может равняться количеству входных файлов переданных утилите на обработу, т.е. теоретически количество параллельных потоков задействованных этой утилитой может быть более чем 32.

Процедура обработки входных даннх специально реализована в виде двух утилит, для того, чтобы пользователь при необходимости оценить полученный промежуточный результат, после чего возможно изменить параметры запуска утилиты, либо подкоректировать полученные утилитой значения в ручном режиме. Наример, если для обработки файла представленного на рис. 2.1. задать опцию `-s`, то на выходе мы получим пустой файл, поскольку содержимое входного файла не соответствует регулярному выражению, определяющему требования к содержимому выходного файла. Пример подобной ситуации показан на скриншоте ниже.


![Пример не соответствия формата](https://github.com/tsf-soft/MergeSort/assets/6228605/58023282-247f-4819-8691-81fe2a663c34)
