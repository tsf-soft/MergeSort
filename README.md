...
Комплект состоит из двух `CLI` утилит: `TsfDataPrepare` и `TsfMergeSort`. Эти утилиты взаимодополняющие и должны быть использваны последовательно, если в качестве входных данных заданы текстовые даныне произвольного формата, такие например, как на рис.2.1. Утилита `TsfDataPrepare` приводит такие данные к формату в котором одно символьное слово занимает одну строку текстового файла, любые пробелы в строках текста отсутствуют. Выходные данные в файлах результата работы этой утилиты - не упорядочены.

Утилита `TsfMergeSort` работает с уже "нормализованными" текстовыми файлами (формат описан выше). Эти "нормализованные" файлы утилита в результате своей работы сводит в один текстовый файл хранящий данные всех входных файлов в упорядоченном (отсортированном) формате (см. опции сортировки). В зависимости от типа заявленных данных (см. тип данных) - выходные данные утилиты `TsfMergeSort` отсортированы либо в числовом порядке, либо в лексикографическом порядке.
...
- `TsfDataPrepare` - для каждого нового входного файла запускает новый параллельный поток исполнения;
- `TsfMergeSort` - последовательно обрабатывает каждый из входных файлов, но каждый входной файл разделяет на несколько фрагментов, - выделение фрагметов из целого большого файла также выполняется в параллельных потоках. После разделения всех входных файлов на фрагменты, сортировка попарным слиянием полученных фрагментов, производится также в параллельных потоках.
Такова специфика распараллеливания процесса обработки каждой из утилит.
