...
Под произвольным текстовым форматом данных подразумевается обычный текстовый файл с расшинением файловой системы Windows `.txt` имеющий как ограниченные, так и "бесконечные" строки текста. Текст может содержать любые символы, в т.ч. пробельные, управляющие (служебные), из полной "палитры" текста, который может быть выражен через кодировку UTF-8. Текст таких файлов может быть как имеющим какой-либо смысл, так и не имеющий какого-либо смысла (рыба-текст).

В качестве примера приведу вариант входного текстового файла содержащего числа в "бесконечных" строках. Этот файл в том числе использовался в качестве одного из тестовых-файов отладки утилит.
